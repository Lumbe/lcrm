...
module AccountsHelper
  ...
  def account_select(options = {})
    options[:selected] = 1               # (@account && @account.id) || 0
    ...
    content_tag(:div, class: 'label') do
      t(:account).html_safe +
        ...
        content_tag(:span, id: 'account_select_title') do
          "(#{t :select_existing} #{t :or} <a href='#' onclick='crm.create_account(); return false;'>#{t :create_new}</a>):".html_safe
        end +

        content_tag(:span, id: 'account_create_title') do
          "(<a href='#' onclick='crm.select_account(); return false;'>#{t :select_existing}</a> #{t :or} #{t :create_new}):".html_safe
        end +
